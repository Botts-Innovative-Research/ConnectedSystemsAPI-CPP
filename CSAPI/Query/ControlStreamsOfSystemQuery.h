...
namespace ConnectedSystemsAPI::Query {
	class ControlStreamsOfSystemQuery : public QueryParameters {
	public:
		/// <summary>
		...
		/// - q=gps,imu
		/// </summary>
		ControlStreamsOfSystemQuery& setQ(const std::string& q) {
			addParameter("q", q);
			return *this;
		}
		...
		/// </summary>
		ControlStreamsOfSystemQuery& setIssueTime(DataModels::TimeExtent const& issueTime) {
		...
		/// Intervals may be bounded or half-bounded (double-dots at start or end).
		/// </summary>
		ControlStreamsOfSystemQuery& setExecutionTime(DataModels::TimeExtent const& executionTime) {
		...
		/// Default: 100
		/// </summary>
		ControlStreamsOfSystemQuery& setLimit(int limit) {

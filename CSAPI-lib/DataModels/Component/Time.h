	...
	namespace DataModels {
		namespace Component {
			...
			struct TimeRegistrar {
				TimeRegistrar() {
					ConnectedSystemsAPI::DataModels::Component::DataComponentRegistry::registerType(
						...
						return std::make_unique<ConnectedSystemsAPI::DataModels::Component::Time>(j.get<ConnectedSystemsAPI::DataModels::Component::Time>());
					}
					);
				}
			};
			static TimeRegistrar timeRegistrar;
			...
			inline void to_json(nlohmann::ordered_json& j, const Time& v) {
				...
				if (v.getReferenceTime()) {
					j["referenceTime"] = v.getReferenceTime().value();
				}
				if (v.getLocalFrame()) {
					j["localFrame"] = v.getLocalFrame().value();
				}
				if (v.getUnitOfMeasure()) {
					j["uom"] = v.getUnitOfMeasure().value();
				}
			}
